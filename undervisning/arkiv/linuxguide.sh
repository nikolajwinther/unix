...
sed -n '1~9p' inputfile                       # Shows every ninth line from inputfile, beginning with the second
...
cat inputfile | tr '\t' '\n'     # replaces all instances of tabulator with newline in inputfile
...
cat numfile | sort -u  # shows only lines that appears once
...
# copies all files from the server named guesstimate from the directory /home/niw/datain/marcposter/
...
for id in "4561 14561 24561 34561 44561 54563"; do wget -O fulltext${id}.txt "http://www.gutenberg.org/files/${id}/${id}-0.txt"; done
